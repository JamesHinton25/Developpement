...
if (((_this select 3) in ["bruce","dive","reb"]
&& playerSide != civilian)) exitWith
{
    hint localize "STR_Shop_NotaCiv";
    closeDialog 0;
};
if (((_this select 3) isEqualTo "reb"
&& !license_civ_rebel)) exitWith
{
    hint parseText localize "STR_Shop_NotaReb";
    closeDialog 0;
};
if (((_this select 3) in ["cop"]
&& playerSide != west)) exitWith
{
    hint localize "STR_Shop_NotaCop";
    closeDialog 0;
};
if (((_this select 3) in ["dive"]
&& !license_civ_dive)) exitWith
{
    hint parseText localize "STR_Shop_NotaDive";
    closeDialog 0;
};
...
if (isClass(missionConfigFile >> "Licenses" >> life_clothing_store)) then {
    ...
    if (!(LICENSE_VALUE(life_clothing_store,_flag))) exitWith {
        hint parseText format[localize "STR_Shop_YouNeed",localize _displayName];
        closeDialog 0;
    };
};
