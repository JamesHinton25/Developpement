...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Reporter ce bug: JAIL_FALLBACK_15 => (time is zero)"; };
...
while {true} do {
    if((round(_time - time)) > 0) then {
        ...
        if(isNil "life_canpay_bail") then {
            hintSilent format["Temps restant :\n %1\n\nPeux payer : %3\nMontant caution : $%2",_countDown,[life_bail_amount] call life_fnc_numberText, "dass"];
        } else {
            hintSilent format["Temps restant :\n %1\n",_countDown];
        };
    };
