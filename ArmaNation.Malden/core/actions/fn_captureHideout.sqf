...
[[0,1],"STR_GNOTF_CaptureSuccess",true,[(group player) getVariable "gang_name"]] remoteExecCall ["life_fnc_broadcast",RCLIENT];

_marqueurName = str(getPos _hideout);
_marqueurEllipse = str(getPos _hideout);
_marqueurBordure = str(getPos _hideout);
_gangName2 = formatText["%1",(group player) getVariable "gang_name"];

if(getMarkerColor _marqueurName == "") then
{
    gang_owner_marqueur = createMarker [_marqueurName, position player];
    _marqueurName setMarkerType "KIA";
    _marqueurName setMarkerColor "ColorBlack";
    _marqueurName setMarkerText str(_gangName2);
    gang_owner_marqueur = createMarker [_marqueurEllipse, position player];
    _marqueurEllipse setMarkerColor "ColorRed";
    _marqueurEllipse setMarkerShape "ellipse";
    _marqueurEllipse setMarkerBrush "FDiagonal";
    _marqueurEllipse setMarkerSize [50,50];
    gang_owner_marqueur = createMarker [_marqueurBordure, position player];
    _marqueurBordure setMarkerColor "ColorBlack";
    _marqueurBordure setMarkerShape "ellipse";
    _marqueurBordure setMarkerBrush "Border";
    _marqueurBordure setMarkerSize [50,50];
    gang_owner_marqueur = "";
}
else
{
