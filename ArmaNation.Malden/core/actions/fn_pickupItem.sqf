...
/*
	@fichier : fn_pickupItem.sqf
	@credits : par JamesHinton.
	@serveur : Malden (ArmaNation)
*/
private ["_itemInfo","_itemName","_illegal","_diff"];
if ((time - life_action_delay) < 2) exitWith {hint parseText localize "STR_NOTF_ActionDelay"; INUSE(_this);};
...
if (playerSide isEqualTo west && _illegal isEqualTo 1) exitWith {
    hint parseText format[localize "STR_NOTF_PickedEvidence",_itemName,[round(ITEM_SELLPRICE(_itemInfo select 0) / 2)] call life_fnc_numberText];
...
if (LIFE_SETTINGS(getNumber,"player_advancedLog") == 1) then {
