...
if (count crew _vehicle isEqualTo 0) then {
    ...
    if (playerSide isEqualTo west) then {
            ...
            if (_vehicle in life_vehicles) then {
                hint parseText format[localize "STR_NOTF_OwnImpounded",[_value] call life_fnc_numberText,_type];
                BANK = BANK - _value;
            } else {
                hint parseText format[localize "STR_NOTF_Impounded",[_value] call life_fnc_numberText,_type];
                BANK = BANK + _value;
            };
