...
switch (_code) do {
    ...
    case 19:
    {
        ...
        && {!(cursorObject getVariable "Escorting")}
        ...
        && {speed cursorObject < 1}) then
        {
            ...
            {
                hint "Cette action requiert une menotte.";
            };
        };
    };
    //knockout (Shift + G)
    case 34: {
        ...
        && isPlayer cursorObject
        && alive cursorObject
        && cursorObject distance player < 4
        && speed cursorObject < 1) then
        {
    ...
    case 24: {
        if (_shift) then {
            if (soundVolume != 1) then {
                1 fadeSound 1;
                hint parseText localize "STR_MISC_soundnormal";
            } else {
                1 fadeSound 0.1;
                hint parseText localize "STR_MISC_soundfade";
            };
        };
    };
    //renfort (F6)
    case 64: {
        if (playerSide == west) then {
            _handled = true;
            [] spawn life_fnc_copBackupCalled;
        };
    };
            ...
            if (_veh isKindOf "House_F" && {playerSide isEqualTo civilian}) then {
                if (_veh in life_vehicles && player distance _veh < 8) then {
                    ...
                    if (_locked isEqualTo 0) then {
                        ...
                        hint parseText localize "STR_House_Door_Lock";
                    } else {
                        _veh setVariable [format["bis_disabled_Door_%1",_door],0,true];
                        _veh animate [format["door_%1_rot",_door],1];
                        hint parseText localize "STR_House_Door_Unlock";
                    };
                };
            } else {
                _locked = locked _veh;
                if (_veh in life_vehicles && player distance _veh < 8) then {
                    if (_locked isEqualTo 2) then {
                        ...
                        hint parseText localize "STR_MISC_VehUnlock";
                        ...
                        hint parseText localize "STR_MISC_VehLock";
