...
if (_value > 999999) exitWith {hint parseText localize "STR_ATM_TransferMax";};
...
if ((_value + _tax) > BANK) exitWith {hint parseText format[localize "STR_ATM_SentMoneyFail",_value,_tax]};

BANK = BANK - (_value + _tax);

[_value,profileName] remoteExecCall ["life_fnc_wireTransfer",_unit];
[] call life_fnc_atmMenu;
hint parseText format[localize "STR_ATM_SentMoneySuccess",[_value] call life_fnc_numberText,_unit getVariable ["realname",name _unit],[_tax] call life_fnc_numberText];
