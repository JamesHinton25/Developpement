...
if (_value > 999999) exitWith {hint parseText localize "STR_ATM_GreaterThan";};
if (_value < 0) exitWith {};
if (!([str(_value)] call TON_fnc_isnumber)) exitWith {hint localize "STR_ATM_notnumeric"};
if (_value > CASH) exitWith {hint localize "STR_ATM_NotEnoughCash"};
if ((group player getVariable ["gbank_in_use_by",player]) != player) exitWith {hint parseText localize "STR_ATM_WithdrawMin"}; //Check if it's in use.
...
hint parseText format[localize "STR_ATM_DepositSuccessG",[_value] call life_fnc_numberText];
